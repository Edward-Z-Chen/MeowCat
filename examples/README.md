...
meowcat prepare-visium           --config config.yaml   # Step 3.5: Visium metadata + embeddings-hist
...
meowcat prepare-xenium           --config config.yaml   # Step 4b: single_super_emb.h5ad → embeddings-hist grid
meowcat train                    --config config.yaml   # Step 5: train model
meowcat predict                  --config config.yaml   # Step 6: full-grid inference
meowcat visualize                --config config.yaml   # Step 6: argmax + intensity maps
meowcat slide                    --config config.yaml   # Step 7: assemble PowerPoint
...
> Note: No `meowcat rctd` or `meowcat prepare-visium` steps — those are Visium-only. `prepare-xenium-batches` merges histology features from `single_super_emb.h5ad` into `adata_cellbin_HistoSweep.h5ad` for **training**; `prepare-xenium` builds the dense `embeddings-hist` grid needed for **prediction** (only depends on `preprocess` output, so it can run any time after Step 3).
...
meowcat prepare-visium           --config config.yaml   # Step 3.5: Visium metadata + embeddings-hist (VIS samples)
meowcat prepare-xenium           --config config.yaml   # Step 3.5x: embeddings-hist (XEN samples)
...
> Important: `prepare-visium` must run before `prepare-xenium-batches` because the Xenium config references `anno-names.txt` generated by the Visium pipeline. `prepare-visium` builds `embeddings-hist` for Visium samples only, so Xenium samples need `prepare-xenium` for their prediction grid.
